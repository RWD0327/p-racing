...
- `dist/scroll-snap.css`: 데스크톱 전체 화면 섹션 레이아웃. `app.js`에서 휠 제스처당 한 섹션을 전환합니다. 방향키·PageUp/Down·Home/End 및 메뉴 이동을 지원합니다. 작은 화면이나 확대 상태에서 넘치는 내용은 섹션 내부에서 읽을 수 있습니다. 모바일은 일반 스크롤을 유지하며 모션 감소 설정에서는 즉시 전환합니다.
